...
#ifdef WIN32
/*Add by Scott to support inet_pton and inet_ntop on Windows XP*/
int _mosquitto_InetPton(int af, const char *src, void *dst)
{
...
const char *_mosquitto_InetNtop(int af, const void *src, char *dst, socklen_t size)
{
  struct sockaddr_storage ss;
...
#ifdef WIN32
	/*Modified by Scott to support inet_pton and inet_ntop on Windows XP*/
	//ipv6_ok = InetPton(AF_INET6, hostname, &ipv6_addr);
	//ipv4_ok = InetPton(AF_INET, hostname, &ipv4_addr);
	ipv6_ok = _mosquitto_InetPton(AF_INET6, hostname, &ipv6_addr);
	ipv4_ok = _mosquitto_InetPton(AF_INET, hostname, &ipv4_addr);
#else
